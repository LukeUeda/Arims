...
char incomingChar; // Start with 'C' or 'D' for operation simulation
char previousChar = '0';
     ...
     if (Serial1.available() > 0) {
        ...
        if (incomingChar == '0' || incomingChar == '10' || incomingChar == 0 || incomingChar == 'S') {
            ...
            while (intersection_nav(row) == 0 && stopFlag != 1 && stateNumber == 0) {
                line_follow();
                // Serial.println(analogRead(A0)); // delete
            }
            ...
            // [4] Follow line back home
            while (intersection_nav(row) == 0 && stopFlag != 1 && stateNumber == 2) {
              line_follow();
            }

            if (stopFlag != 1 && stateNumber == 3){
              at_home();
            }

        }
      if(incomingChar == 'Z' && previousChar != incomingChar){
      disco();
      stop();
    }
}  // end if main loop
...
int intersection_nav(int intersecTarget){
  ...
  if (intersecCount == intersecTarget) {
    ...
    ++stateNumber;    // Change state if needed
    return 1;
  } else {
    return 0;
  }
...
void shelf_movement_right(char op) {
    ...
    if (op == 'C') {
      ...
      // Lift to lower height
      for(int i = 0; i < liftRaiseTime*3; ++i){
        if (col == 2){
          scissor_drive(M_Low);
        }
        else if (col == 3){
      ...
      // Move to shelf
      delay(1000);
      translate_right();  // move command
      delay(moveTime); // Wait for the specified move time
      stop();
      delay(500);

      // Lift to upper height
      for(int i = 0; i < liftRaiseTime*2; ++i){
        if (col == 2){
          scissor_drive(M_High);
        }
        else if (col == 3){
          scissor_drive(T_High);
        }
      }
      analogWrite(11, 255);

      // Container from shelf to ARIMS
      delay(500);
    ...
    // ++++++++++++ Place sequence ++++++++++++ 
      ...
      // Lift to upper height
      for(int i = 0; i < liftRaiseTime*7; ++i){
        if (col == 2){
          scissor_drive(M_High);
         }
         else if (col == 3){
...
void at_home(){
  ...
  stop();
  rotate_left();  // Spin back to correct direction
  delay(4500);    // 4500ms approximatly for 180deg
  stop();

  for(int i = 0; i < 20; i++){
    line_follow();  // Short line follow to straighten.
  }
  stop();
...
void scissor_drive(int target){

  // PID Values, not great, testing hard.
  const int KP1 = 0.6;
  const int KD1 = 0.0;

  int verticalPosition = analogRead(A0);  // read potentiometer
  int direction = 0;

  // PID calculations
  int error = verticalPosition - target;
  int absError = abs(error);
  int adjust = KP*absError + KD*(absError-last_error1);
  last_error1 = absError;
 
  // adjusted motor speeds
  int outputSpeed = constrain(255 - adjust,0,80);
  ...
  // Down Direction
    ...
    digitalWrite(30, HIGH); // SCISSOR DOWN
    direction = -1;
  }
  ...
  */
  ...
  if (absError <= 5){
    analogWrite(11, 255); // Stop if at target
    // Serial.println("At Target");
  }
  else{
    analogWrite(11, outputSpeed); // Speed release
  }
}
